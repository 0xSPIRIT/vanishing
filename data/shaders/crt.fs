...
vec4 vignette(vec2 tex_coord, vec4 col) {
    ...
    if (vignette_intensity == 0.0) return col;
...
void main() {
    ...
    if (do_spin == 1) {
        tex_coord.y += mod(time * 3.0, 1.0);
        tex_coord.x += 0.25 * tex_coord.y * tex_coord.y;
    }

    tex_coord.y = mod(tex_coord.y, 1.0);
    ...
    bool not_at_edges = gl_FragCoord.x > 1.0 && gl_FragCoord.y > 1.0 &&
                        gl_FragCoord.x < 191.0 && gl_FragCoord.y < 159.0;

    if (not_at_edges && abberation_intensity > 0.0) {
    ...
    if (do_scanline_effect == 1) {
        ...
        float scanline_count = 80.0;
        int y_i = int(round(y * scanline_count));

        if (y_i % 2 == 0) {
            finalColor = mix(finalColor, vec4(0.0,0.0,0.0,1.0), scanline_alpha);
        }
    }
}
